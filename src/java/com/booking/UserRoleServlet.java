...
public class UserRoleServlet extends HttpServlet {
    ...
    // Factory Pattern - UserRole Creation
    public static class UserRoleFactory {
        public static UserRole createUserRole(int roleId, String roleName) {
        ...
        public static UserRole createUserRoleByName(String roleName) {
            UserRole role = new UserRole();
    ...
    // Strategy Pattern - Role Validation
    public interface RoleValidationStrategy {
        boolean validateRole(String roleName);
        String getValidationMessage();
    }
    
    public static class BasicRoleValidationStrategy implements RoleValidationStrategy {
        @Override
    ...
    public static class AdvancedRoleValidationStrategy implements RoleValidationStrategy {
        @Override
        public boolean validateRole(String roleName) {
            if (roleName == null || roleName.trim().isEmpty()) {
                return false;
            }
            // Check for valid role names (only letters, numbers, and underscores)
            return roleName.matches("^[A-Za-z0-9_]+$");
        }
        
        @Override
        public String getValidationMessage() {
            return "Role name must contain only letters, numbers, and underscores";
        }
    }
    
    // Template Method Pattern - Role Management Operations
    public abstract static class RoleOperationTemplate {
        protected abstract void performOperation(HttpServletRequest request, HttpServletResponse response, HttpSession session) throws ServletException, IOException;
        protected abstract void validateInput(HttpServletRequest request) throws Exception;
        protected abstract void logOperation(String operation, String details);
        
        public final void execute(HttpServletRequest request, HttpServletResponse response, HttpSession session) throws ServletException, IOException {
            try {
                validateInput(request);
                performOperation(request, response, session);
                logOperation("SUCCESS", "Operation completed successfully");
            } catch (Exception e) {
                logOperation("ERROR", "Operation failed: " + e.getMessage());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        // Check if user is admin or manager (admin and manager can manage user roles)
        String currentUserRole = (String) session.getAttribute("role");
        if (!"ADMIN".equals(currentUserRole) && !"MANAGER".equals(currentUserRole)) {
            response.sendRedirect("dashboard.jsp?error=Access denied. Only administrators and managers can manage user roles.");
            return;
        }
    ...
    private void handleCreateUserRole(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        try {
            ...
            if (success) {
                System.out.println("User role created successfully: " + roleName);
                response.sendRedirect("user_role.jsp?message=User role created successfully.");
            } else {
                System.err.println("User role creation failed: " + roleName);
                response.sendRedirect("user_role.jsp?error=Failed to create user role.");
            }

        } catch (Exception e) {
            System.err.println("User role creation error: " + e.getMessage());
    ...
    private void handleUpdateUserRole(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        try {
            ...
            if (success) {
                System.out.println("User role updated successfully: " + roleName);
                response.sendRedirect("user_role.jsp?message=User role updated successfully.");
            } else {
                System.err.println("User role update failed: " + roleName);
                response.sendRedirect("user_role.jsp?error=Failed to update user role.");
            }

        } catch (Exception e) {
            System.err.println("User role update error: " + e.getMessage());
    ...
    private void handleDeleteUserRole(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        try {
            ...
            if (roleId <= 4) { // Prevent deletion of core roles (ADMIN, MANAGER, STAFF, CUSTOMER)
            ...
            if (success) {
                System.out.println("User role deleted successfully: ID " + roleId);
                response.sendRedirect("user_role.jsp?message=User role deleted successfully.");
            } else {
                System.err.println("User role deletion failed: ID " + roleId);
                response.sendRedirect("user_role.jsp?error=Failed to delete user role.");
            }

        } catch (Exception e) {
            System.err.println("User role deletion error: " + e.getMessage());
    ...
    private void handleViewUserRole(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        try {
            ...
            System.err.println("User role view error: " + e.getMessage());
    ...
    private void handleListUserRoles(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        try {
            ...
            System.err.println("User role list error: " + e.getMessage());
